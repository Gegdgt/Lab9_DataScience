...
# Predicción ARIMA
st.subheader("Predicción ARIMA para Precios GLP")
...
pred_arima_glp_dates = pd.date_range(start=precios_glp_ts.index[-1], periods=12, freq='M')

# Visualización 2: Predicción ARIMA con Plotly (Dinámica)
fig_arima = go.Figure()
fig_arima.add_trace(go.Scatter(x=precios_glp_ts.index, y=precios_glp_ts, mode='lines', name='Datos Observados'))
fig_arima.add_trace(go.Scatter(x=pred_arima_glp_dates, y=pred_arima_glp, mode='lines', name='Predicción ARIMA', line=dict(dash='dash')))
fig_arima.update_layout(title='Predicción ARIMA para Precios GLP', xaxis_title='Fecha', yaxis_title='Precio (Q)')
